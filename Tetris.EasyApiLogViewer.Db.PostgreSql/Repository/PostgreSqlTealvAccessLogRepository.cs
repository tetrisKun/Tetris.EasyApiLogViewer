...
/// PostgreSQL访问日志仓储实现
/// </summary>
public class PostgreSqlTealvAccessLogRepository : ITealvAccessLogRepository
{
    private readonly string _connectionString;
    private readonly string _tableName;
    private readonly ILogger<PostgreSqlTealvAccessLogRepository> _logger;

    /// <summary>
    /// 初始化 PostgreSQL PostgreSqlTealvAccessLogRepository仓储
    /// </summary>
    /// <param name="options">配置选项</param>
    /// <param name="logger">日志记录器</param>
    public PostgreSqlTealvAccessLogRepository(
        IOptions<TealvOptions> options,
        ILogger<PostgreSqlTealvAccessLogRepository> logger)
    {
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        var createTableSql = $@"
            ...
            CREATE INDEX IF NOT EXISTS ""IX_{_tableName}_Timestamp"" ON {fullTableName} (""Timestamp"" DESC);
            CREATE INDEX IF NOT EXISTS ""IX_{_tableName}_StatusCode"" ON {fullTableName} (""StatusCode"");";
    ...
    public async Task<LogQueryResult> QueryAsync(LogQueryParameters parameters, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrEmpty(parameters.Method))
        {
            ...
            sqlParams.Add(new NpgsqlParameter("@Method", parameters.Method.ToUpper()));
        }

        if (!string.IsNullOrEmpty(parameters.Path))
        {
            conditions.Add("\"Path\" LIKE @Path");
            sqlParams.Add(new NpgsqlParameter("@Path", $"%{parameters.Path}%"));
        }

        if (parameters.StatusCode.HasValue)
        {
            conditions.Add("\"StatusCode\" = @StatusCode");
            sqlParams.Add(new NpgsqlParameter("@StatusCode", parameters.StatusCode.Value));
        }

        if (parameters.StartDate.HasValue)
        {
            conditions.Add("\"Timestamp\" >= @StartDate");
            sqlParams.Add(new NpgsqlParameter("@StartDate", parameters.StartDate.Value));
        }

        if (parameters.EndDate.HasValue)
        {
            conditions.Add("\"Timestamp\" <= @EndDate");
            sqlParams.Add(new NpgsqlParameter("@EndDate", parameters.EndDate.Value));
        }
    ...
    public async Task<LogStatistics> GetStatisticsAsync(DateTime? startDate = null, DateTime? endDate = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            conditions.Add("\"Timestamp\" >= @StartDate");
            sqlParams.Add(new NpgsqlParameter("@StartDate", startDate.Value));
        }

        if (endDate.HasValue)
        {
            conditions.Add("\"Timestamp\" <= @EndDate");
            sqlParams.Add(new NpgsqlParameter("@EndDate", endDate.Value));
        }
        ...
        // 总数和平均时间
        var statsSql = $"SELECT COUNT(*), AVG(CAST(\"\"Duration\"\" AS DOUBLE PRECISION)) FROM {fullTableName} {whereClause}";
    ...
    public async Task<int> PurgeOldLogsAsync(int retentionDays, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Purged {Count} old access logs from PostgreSQL", deletedCount);
