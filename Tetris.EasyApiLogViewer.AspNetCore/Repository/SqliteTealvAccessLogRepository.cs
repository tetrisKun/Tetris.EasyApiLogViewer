...
/// SQLite访问日志仓储实现
/// </summary>
public class SqliteTealvAccessLogRepository : ITealvAccessLogRepository
{
    private readonly string _connectionString;
    private readonly string _tableName;
    private readonly ILogger<SqliteTealvAccessLogRepository> _logger;
    ...
    public SqliteTealvAccessLogRepository(
        IOptions<TealvOptions> options,
        ILogger<SqliteTealvAccessLogRepository> logger)
    {
        ...
        _tableName = opts.AccessLogTableName;
        _logger = logger;
    }
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        var createTableSql = $@"
            ...
            CREATE INDEX IF NOT EXISTS idx_{_tableName}_timestamp ON {_tableName}(Timestamp DESC);
            CREATE INDEX IF NOT EXISTS idx_{_tableName}_status_code ON {_tableName}(StatusCode);
        ";
    ...
    public async Task InsertAsync(TealvAccessLogs entry, CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.AddWithValue("@Timestamp", entry.Timestamp.ToString("yyyy-MM-dd HH:mm:ss.fff"));
    ...
    public async Task<LogQueryResult> QueryAsync(LogQueryParameters parameters, CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrEmpty(parameters.Method))
        {
            ...
            sqlParams.Add(new SqliteParameter("@Method", parameters.Method.ToUpper()));
        }

        if (!string.IsNullOrEmpty(parameters.Path))
        {
            conditions.Add("Path LIKE @Path");
            sqlParams.Add(new SqliteParameter("@Path", $"%{parameters.Path}%"));
        }

        if (parameters.StatusCode.HasValue)
        {
            conditions.Add("StatusCode = @StatusCode");
            sqlParams.Add(new SqliteParameter("@StatusCode", parameters.StatusCode.Value));
        }

        if (parameters.StartDate.HasValue)
        {
            conditions.Add("Timestamp >= @StartDate");
            sqlParams.Add(new SqliteParameter("@StartDate", parameters.StartDate.Value.ToString("yyyy-MM-dd HH:mm:ss")));
        }

        if (parameters.EndDate.HasValue)
        {
            conditions.Add("Timestamp <= @EndDate");
            sqlParams.Add(new SqliteParameter("@EndDate", parameters.EndDate.Value.ToString("yyyy-MM-dd HH:mm:ss")));
        }
    ...
    public async Task<LogStatistics> GetStatisticsAsync(DateTime? startDate = null, DateTime? endDate = null, CancellationToken cancellationToken = default)
    {
        ...
        {
            conditions.Add("Timestamp >= @StartDate");
            sqlParams.Add(new SqliteParameter("@StartDate", startDate.Value.ToString("yyyy-MM-dd HH:mm:ss")));
        }

        if (endDate.HasValue)
        {
            conditions.Add("Timestamp <= @EndDate");
            sqlParams.Add(new SqliteParameter("@EndDate", endDate.Value.ToString("yyyy-MM-dd HH:mm:ss")));
        }
        ...
        // 总数和平均时间
        var statsSql = $"SELECT COUNT(*), AVG(Duration) FROM {_tableName} {whereClause}";
    ...
    {
        var cutoffDate = DateTime.UtcNow.AddDays(-retentionDays).ToString("yyyy-MM-dd HH:mm:ss");

        await using var connection = await GetConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();

        command.CommandText = $"DELETE FROM {_tableName} WHERE Timestamp < @CutoffDate";
        command.Parameters.AddWithValue("@CutoffDate", cutoffDate);

        var deletedCount = await command.ExecuteNonQueryAsync(cancellationToken);
        _logger.LogInformation("Purged {Count} old access logs", deletedCount);
    ...
    private static TealvAccessLogs MapToAccessLog(IDataReader reader)
    {
        return new TealvAccessLogs
        {
            ...
            Timestamp = DateTime.TryParse(reader.IsDBNull(1) ? null : reader.GetString(1), out var ts) ? ts : DateTime.MinValue,
